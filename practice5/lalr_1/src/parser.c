...
parser_table *load_parser_table_csv(const grammar *g, const char *input_path)
{
    if (g == NULL || input_path == NULL)
    {
        return NULL;
    }
    ...
    while (fgets(line, sizeof(line), file))
    {
        char section[64];
        int state_id = -1;
        if (sscanf(line, "%[^,],%d,", section, &state_id) == 2)
        {
            if (state_id > max_state) {
                max_state = state_id;
            }
        }
    }
    ...
    table->action_table = (parser_action *)malloc((size_t)table->num_states * (size_t)table->num_terminals_with_eof * sizeof(parser_action));
    table->goto_table = (int *)malloc((size_t)table->num_states * (size_t)table->num_non_terminals * sizeof(int));
    ...
    while (fgets(line, sizeof(line), file))
    {
        ...
        *comma2 = '\0';
        ...
        if (!comma4) continue;
        *comma4 = '\0';
        strcpy(action_type, ptr);
        ptr = comma4 + 1;

        value = atoi(ptr);

        if (strcmp(section, "ACTION") == 0) {
            int terminal_id = -1;
            if (strcmp(symbol, "$") == 0) {
                terminal_id = g->num_terminals;
            } else {
                for (int i = 0; i < g->num_terminals; i++) {
                    if (strcmp(g->terminals[i].symbol, symbol) == 0) {
                        terminal_id = i;
                        break;
                    }
                }
            }
            if (terminal_id >= 0 && terminal_id < table->num_terminals_with_eof) {
                parser_action act = make_error_action();
                if (strcmp(action_type, "SHIFT") == 0) act = make_shift_action(value);
                else if (strcmp(action_type, "REDUCE") == 0) act = make_reduce_action(value);
                else if (strcmp(action_type, "ACCEPT") == 0) act = make_accept_action();
                
                table->action_table[action_index(table, state_id, terminal_id)] = act;
            }
        } else if (strcmp(section, "GOTO") == 0) {
            int nt_id = -1;
            for (int i = 0; i < g->num_non_terminals; i++) {
                if (strcmp(g->non_terminals[i].symbol, symbol) == 0) {
                    nt_id = i;
                    break;
                }
            }
            if (nt_id >= 0 && nt_id < table->num_non_terminals) {
                table->goto_table[goto_index(table, state_id, nt_id)] = value;
            }
        }
    }
